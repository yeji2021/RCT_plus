...
def peak_signal_noise_ratio(outputs, targets):
    outputs = torch.unbind(0.5 * (outputs + 1.0), dim=0)
    targets = torch.unbind(0.5 * (targets + 1.0), dim=0)
...
def structural_similarity(outputs, targets):
    ...
    targets = 0.5 * (targets + 1.0)
    ssim = kornia.metrics.ssim(outputs, targets, window_size=11, max_val=1.0)
    ssim = torch.mean(ssim)
    return ssim

def perceptual_similarity(lpips, outputs, targets):
    l_vgg = lpips(outputs, targets)
    return torch.mean(l_vgg)
